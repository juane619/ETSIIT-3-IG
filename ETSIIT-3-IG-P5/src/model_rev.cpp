...
void Rev::generarRevolucion() {
    ...
    for (unsigned j = 0; j < n_ver_in * lados_long; j += n_ver_in) {
        for (unsigned i = j; i < n_ver_in - 1 + j; i++) {
            caras.push_back(Cara(i, i + 1, i + n_ver_in));
            caras.push_back(Cara(i+1, i + n_ver_in + 1, i + n_ver_in));
        }
    }
...
void Rev::pintarTapas() {
    if (!tapas_pintadas) {
        ...
        for (unsigned j = 0; j < n_ver_in * lados_long; j += n_ver_in) {
            caras.push_back(Cara(j, j + n_ver_in, (vertices.size() - 2)));
        }

        for (unsigned j = 0; j < n_ver_in * lados_long; j += n_ver_in) {
            caras.push_back(Cara((vertices.size() - 1), n_ver_in + n_ver_in - 1 + j,  j + n_ver_in - 1));
        }
