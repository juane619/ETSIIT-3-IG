...
void Rev::generarRevolucion() {
    ...
    for (unsigned int j = 1; j <= lados_long; j++) {
        ...
        for (unsigned int i = 0; i < n_ver_in; i++) {            
vertices.push_back(Punto3D(
    ...
    for (int j = 0; j < n_ver_in * lados_long; j += n_ver_in) {
        for (unsigned int i = 0 + j; i < n_ver_in - 1 + j; i++) {
...
void Rev::comprobarOrden(){
	...
	for(int i=1; i<vertices.size()-1 && !encontrado; i++){
