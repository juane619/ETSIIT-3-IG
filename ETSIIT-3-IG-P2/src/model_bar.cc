...
Bar::Bar(const std::string &file_name_p, int lad_long_p) {
    _file_ply file_ply;
...
void Bar::generarBarRev(int lados_long, bool pintar_tapas) {
    ...
    for (unsigned int j = 1; j <= lados_long; j++) {
    ...
    for (int j = 0; j < n_ver * lados_long; j += n_ver) {
        for (unsigned int i = 0 + j; i < n_ver - 1 + j; i++) {
